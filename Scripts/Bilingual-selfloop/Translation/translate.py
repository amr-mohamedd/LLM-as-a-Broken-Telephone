...
from config import prompt3 as prompt, hf_auth_token, num_texts, num_translations, lang_dict_map, log_dir, output_dir, logging_format, logging_level, get_llama_config, get_mistral_config
import numpy as np
...
def run_translation_pipeline(model_names, dataset_name, cuda_device, language_sequence, shuffle, default_file_path):
    ...
    if "llama" in model_names[0].lower():
    ...
    if "mistral" in model_names[0].lower():
