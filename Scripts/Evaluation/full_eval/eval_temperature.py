...
def get_titles(dataset_name, results_path):
    ...
    return dict(zip(merged['0_en'].str[:100].apply(sanitize_text), merged['title']))
...
def extract_and_save(file_path, nested_results):
    ...
    output_dir = f"./../../../Results/Evaluation/Ablations/temperature variation/"
...
def process_file_to_nested_dict(
    ...
    dataset_name  = "news2024"
    model_name    = file_path.split('/')[-3]
    setting_name  = file_path.split('/')[-4]
    output_dir    = f"./../../../Results/Evaluation/Ablations/temperature variation/{metric_dir}"
    ...
    for i in range(num_passages):
        try:
            ...
            title = titles[reference_text[:100].strip()]
