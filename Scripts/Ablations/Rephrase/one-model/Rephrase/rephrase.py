...
import time
import os
from config import prompt3 as prompt, hf_auth_token, log_dir, output_dir, logging_format, logging_level, get_llama_config, get_mistral_config
import numpy as np
torch.manual_seed(0)
random.seed(0)
np.random.seed(0)
...
def run_rephrasing_pipeline(model_name, dataset_name, cuda_device, num_rephrasings, default_file_path):
    """
    Run the rephrasing pipeline to rephrase texts iteratively.

    Args:
        model_name (str): Name of the model to use for rephrasing
    ...
    if "llama" in model_name.lower():
        ...
        llama_tokenizer = AutoTokenizer.from_pretrained(model_name, token=hf_auth_token)
    ...
    if "mistral" in model_name.lower():
        logging.info("Loading tokenizer for model: %s", model_name)
        mistral_tokenizer = AutoTokenizer.from_pretrained(model_name, token=hf_auth_token)
        logging.info("Loading model: %s", model_name)
    ...
    def rephrase_message(text):
        ...
        return rephrased_text.replace(prompt, "").strip()

    def iterative_rephrasing(passage, n):
        """Perform iterative rephrasing."""
        rephrased_passage = passage
        rephrased_passages = {"prompt": prompt, "0_en": passage}
        for i in range(1, n+1):
            logging.info("Iteration %d/%d: Rephrasing text", i, n)
            rephrased_passage = rephrase_message(rephrased_passage)
    ...
    output_file_path = os.path.join(
        output_dir,
        f'{dataset_name}/rephrased_passages_{dataset_name}_{model_name.split("/")[-1]}_rephrase_{file_timestamp}.json'
    )
...
if __name__ == "__main__":
    ...
    log_filename = f"{log_dir}/rephrase_{args.model_name.replace('/', '_')}_rephrase_{args.dataset_name}_{timestamp}_{args.random_seed}.log"
