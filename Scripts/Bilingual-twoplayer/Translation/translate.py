...
from config import prompt3 as prompt, hf_auth_token, num_texts, num_translations, lang_dict_map, default_file_path, log_dir, output_dir, logging_format, logging_level, get_llama_config, get_mistral_config
import numpy as np  
...
def run_translation_pipeline(model_names, dataset_name, cuda_device, language_sequence, shuffle, default_file_path):
    """
    Run the tranxslation pipeline to translate texts through multiple languages.
    ...
    tokenizer = AutoTokenizer.from_pretrained(llama_model_name, token=hf_auth_token)
    ...
    # Load Model and Tokenizer for Mistral
    ...
    def translate_message(text, src_lang, tgt_lang):
        ...
        if "llama" in selected_model.lower():
            ...
            translated_text_passage = get_response_llama(messages)
    ...
    output_file_path = f'{output_dir}/translated_passages_{dataset_name}_{"_".join(model_names).replace("/", "_")}_trans_{"_".join(language_sequence)}_{file_timestamp}_{default_file_path}.json'
...
if __name__ == "__main__":
    ...
    log_filename = f"{log_dir}/translation_{'_'.join(args.model_names).replace('/', '_')}_trans_{args.dataset_name}_{'_'.join(args.language_sequence)}_{timestamp}_{args.default_file_path}.log"
